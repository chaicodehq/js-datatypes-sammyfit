...
export function validateForm(formData) {
  ...
  if (typeof formData.phone !== 'string' || formData.phone.length !== 10 || !['6', '7', '8', '9'].includes(formData.phone[0]) || ![...formData.phone].every(char => char >= '0' && char <= '9')) {
    errors.phone = "Invalid Indian phone number";
  }
  let age;
  if(typeof formData.age === "string") { age = parseInt(formData.age); }
  else { age = formData.age; }
  if (isNaN(age) || age < 16 || age > 100 || !Number.isInteger(age)) {
    errors.age = "Age must be an integer between 16 and 100";
  }
  if (typeof formData.pincode !== 'string' || formData.pincode.length !== 6 || formData.pincode.startsWith('0') || ![...formData.pincode].every(char => char >= '0' && char <= '9')) {
